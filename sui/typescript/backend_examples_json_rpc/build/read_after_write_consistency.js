...
import { WalletClient, KeyClient, ShinamiWalletSigner, createSuiClient, buildGaslessTransaction } from "@shinami/clients/sui";
...
const signer = new ShinamiWalletSigner(INVISIBLE_WAL_ID, walletClient, INVISIBLE_WAL_SECRET, keyClient);
...
//
async function buildAndSubmitGaslessTx(signer, tx) {
...
//
async function parseTxResponse(response, print = true) {
    const objectChanges = response.objectChanges;
    if (objectChanges) {
        for (var change in objectChanges) {
            const objChange = objectChanges[change];
