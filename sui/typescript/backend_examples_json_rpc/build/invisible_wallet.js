...
import { WalletClient, KeyClient, GasStationClient, buildGaslessTransaction, ShinamiWalletSigner } from "@shinami/clients/sui";
import { Transaction } from "@mysten/sui/transactions";
import { verifyPersonalMessageSignature } from '@mysten/sui/verify';
import { SuiClient, getFullnodeUrl } from "@mysten/sui/client";
// 2. Copy your access key value. Must have Testnet rights to all Shinami services.
const GAS_AND_WALLET_KEY = "sui_testnet_e7d3fa60be1ad2e4649a4e79e910c04d"; //"{{API_KEY_VALUE}}";
...
const signer = new ShinamiWalletSigner(WALLET_ONE_ID, walletClient, WALLET_ONE_SECRET, keyClient);
...
const gaslessTx = await buildGaslessTransaction(await mintSwordTransaction(), { sui: nodeClient });
...
async function sponsorSignExecuteInOneRequest(signer, gaslessTx) {
    const sponsorSignAndExecuteResponse = await signer.executeGaslessTransaction(gaslessTx);
...
async function sponsorSignExecuteInThreeRequests(signer, gaslessTx) {
    ...
    const sponsoredResponse = await gasStationClient.sponsorTransaction(gaslessTx // by not setting gaslessTx.gasBudget we take advantage of Shinami auto-budgeting
    );
    // 2. Sign the transaction (the Invisible Wallet is the sender)
    const senderSignature = await signer.signTransaction(sponsoredResponse.txBytes);
...
async function mintSwordTransaction() {
...
async function clockMoveCallTransaction() {
    const tx = new Transaction();
    tx.moveCall({
...
async function signAndVerifyPersonalMessage(signer) {
    ...
    const signature = await signer.signPersonalMessage(messageAsBase64String);
