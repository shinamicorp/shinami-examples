import { Aptos, AptosConfig, Network, SingleKeyAccount, MoveString, SigningSchemeInput } from "@aptos-labs/ts-sdk";
...
//
const committedTransaction = await sponsorTransactionSimple();
...
async function sponsorTransactionSimple() {
    // 1. Set up our sender.
...
async function sponsorTransactionMultiAgent() {
...
async function sponsorAndSubmitSignedTransactionSimple() {
    // 1. Set up our sender.
    const sender = await generateSingleKeyAccountEd25519();
    // 2. Build a simple transaction.
    ...
    return await gasStationClient.sponsorAndSubmitSignedTransaction(transaction, senderAuthenticator);
}
// Build, sign, then sponsor and submit a multiAgent transaction
async function sponsorAndSubmitSignedTransactionMultiAgent() {
    // 1. Generate two funded accounts to act as sender and secondary signer
    const sender = await generateSingleKeyAccountEd25519(true);
    const secondarySigner = await generateSingleKeyAccountEd25519(true);
    // 2. Build a multiAgent transaction
...
async function generateSingleKeyAccountEd25519(fund = false) {
    const account = SingleKeyAccount.generate({ scheme: SigningSchemeInput.Ed25519 });
    if (fund) {
...
async function buildSimpleMoveCallTransaction(sender, expirationSeconds) {
...
async function buildMultiAgentScriptTransaction(sender, secondarySigner, expirationSeconds) {
